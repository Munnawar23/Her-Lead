...
const ProcessSection = () => {
  ...
  useGSAP(() => {
    ...
    const cards = sliderRef.current?.querySelectorAll('.group')
    ...
    cards?.forEach(card => {
      card.addEventListener('mouseenter', () => gsap.to(cursorRef.current, { scale: 0, opacity: 0 }))
      card.addEventListener('mouseleave', () => gsap.to(cursorRef.current, { scale: 1, opacity: 1 }))
    })
    ...
    return () => {
      ...
      cards?.forEach(card => {
        card.removeEventListener('mouseenter', () => gsap.to(cursorRef.current, { scale: 0, opacity: 0 }))
        card.removeEventListener('mouseleave', () => gsap.to(cursorRef.current, { scale: 1, opacity: 1 }))
      })
  ...
  return (
    <section ref={sectionRef} className="relative w-full bg-bg-light pt-16 lg:pt-24 pb-8 lg:pb-12 overflow-hidden flex flex-col justify-center">
      <div ref={cursorRef} className="fixed top-0 left-0 w-24 h-24 bg-accent-blue rounded-full items-center justify-center text-white font-black text-[10px] uppercase tracking-widest pointer-events-none z-50 -translate-x-1/2 -translate-y-1/2 hidden lg:flex opacity-0 scale-0 transition-opacity duration-300">
        Drag
      </div>
      ...
      <div className="relative z-10 w-full">
        <div className="px-6 md:px-12 lg:px-20 mb-12 lg:mb-20">
          <div className="process-reveal inline-flex items-center gap-3 px-4 py-2 rounded-full bg-text/5 border border-text/10 mb-8">
            <span className="w-2 h-2 rounded-full bg-secondary animate-pulse" />
            <span className="text-[10px] md:text-sm font-heading font-black uppercase tracking-[0.25em] text-text">Our Workflow</span>
          </div>

          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
            <h2 className="process-reveal text-4xl md:text-5xl lg:text-[4.5vw] font-heading font-black uppercase leading-[0.8] tracking-tighter text-text">
              The <span className="text-secondary italic">HerLead</span> <br /> Blueprint.
            </h2>

            <div className="process-reveal flex items-center gap-4">
              <button onClick={scrollPrev} disabled={activeStep === 0} className={`w-14 h-14 rounded-full border border-text/10 flex items-center justify-center text-text transition-all duration-500 cursor-pointer active:scale-90 ${activeStep === 0 ? 'opacity-20' : 'hover:bg-secondary hover:text-white'}`}>
                <ArrowLeft size={20} />
              </button>
              <button onClick={scrollNext} disabled={activeStep === totalItems - 1} className={`w-14 h-14 rounded-full bg-secondary flex items-center justify-center text-white transition-all duration-500 cursor-pointer active:scale-90 ${activeStep === totalItems - 1 ? 'opacity-20' : 'hover:bg-text hover:text-white'}`}>
                <ArrowRight size={20} />
              </button>
            </div>
          </div>
            ...
            {steps.map((step) => (
              <div key={step.number} className="group relative shrink-0 w-[310px] md:w-[400px] lg:w-[450px] h-[420px] md:h-[500px] lg:h-[550px] bg-secondary border border-white/10 rounded-[3rem] p-10 md:p-14 flex flex-col justify-between transition-all duration-700 hover:border-white/40 overflow-hidden lg:cursor-none">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-2xl md:text-4xl lg:text-4xl font-heading font-black text-white mb-6 uppercase tracking-tight group-hover:translate-x-2 transition-transform duration-500">
                    {step.title}
                  </h3>

                  <p className="text-base md:text-xl font-body font-bold text-white leading-relaxed mb-8 transition-colors">
                    {step.description}
                  </p>
                </div>
                ...
                <div className="relative z-10 mt-auto">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <MoveRight className="text-white group-hover:text-white transition-all duration-500" size={20} />
                  </div>
                </div>
              </div>
            ))}

            <div className="shrink-0 w-[310px] md:w-[400px] lg:w-[450px] h-[420px] md:h-[500px] lg:h-[550px] bg-text rounded-[3rem] p-10 md:p-14 flex flex-col items-center justify-center text-center gap-10 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                <h3 className="text-3xl md:text-5xl lg:text-6xl font-heading font-black text-white uppercase tracking-tighter leading-none mb-4">
                  Scale <br /> Faster.
                </h3>
                <p className="text-white font-body font-bold uppercase tracking-widest text-[11px] mb-10 max-w-[200px] mx-auto">
                  Ready to build your legacy?
                </p>
                <button className="px-10 py-4 md:px-12 md:py-5 bg-secondary text-white rounded-2xl font-heading font-black uppercase tracking-widest text-xs md:text-sm hover:scale-105 transition-all active:scale-95 cursor-pointer">
                  Book Call
                </button>
              </div>
